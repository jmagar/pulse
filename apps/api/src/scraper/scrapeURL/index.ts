...
export async function scrapeURL(
  ...
  return withSpan("scrape.pipeline", async span => {
    ...
    try {
      ...
      const MAX_SCRAPE_LOOP_ITERATIONS = 10;
      let scrapeLoopIterations = 0;
      ...
      while (true) {
        try {
          ...
          scrapeLoopIterations++;

          if (scrapeLoopIterations >= MAX_SCRAPE_LOOP_ITERATIONS) {
            meta.logger.error(
              "Maximum scrape loop iterations reached, preventing infinite loop",
              { scrapeLoopIterations, featureFlags: Array.from(meta.featureFlags) },
            );
            throw new Error(
              `Scrape loop exceeded maximum iterations (${MAX_SCRAPE_LOOP_ITERATIONS}). This may indicate a configuration issue with engine selection.`,
            );
          }
          ...
          if (
            ...
            meta.logger.debug(
              ...
              { error, existingFlags: meta.featureFlags, iteration: scrapeLoopIterations },
            );
            ...
            if (meta.pdfPrefetch !== undefined) {
              ...
              meta.logger.debug(
                ...
                { iteration: scrapeLoopIterations },
              );
              meta.featureFlags = new Set(
