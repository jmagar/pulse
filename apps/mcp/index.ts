...
function validateEnvironment(): void {
  ...
  if (optimizeFor && !["cost", "speed"].includes(optimizeFor)) {
    logError(
      "validateEnvironment",
      new Error(`Invalid OPTIMIZE_FOR value: ${optimizeFor}`),
      {
...
async function main(): Promise<void> {
  ...
  if (process.env.SKIP_HEALTH_CHECKS !== "true") {
    ...
    const failedChecks = healthResults.filter(
      (result: HealthCheckResult) => !result.success,
    );
    if (failedChecks.length > 0) {
      logError(
        "healthCheck",
        new Error("Authentication health check failures"),
        {
  ...
  const server = app.listen(port, async () => {
    const serverConfig = {
      ...
      oauthRedirectUri:
        process.env.MCP_GOOGLE_REDIRECT_URI || process.env.GOOGLE_REDIRECT_URI,
