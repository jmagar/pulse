...
describe("WebhookPostgresStorage", () => {
    ...
    it("should call webhook API and transform response to ResourceData[]", async () => {
      ...
      global.fetch = vi.fn(
        async (url: string | URL | Request, options?: RequestInit) => {
          expect(url).toBe(
            "http://pulse_webhook:52100/api/content/by-url?url=https%3A%2F%2Fexample.com&limit=10",
          );
  ...
  describe("read", () => {
    ...
    it("should throw error when content not found", async () => {
      ...
      await expect(storage.read("webhook://999")).rejects.toThrow(
        "Resource not found",
      );
    });
  ...
  describe("exists", () => {
    ...
    it("should throw error for invalid URI format", async () => {
      await expect(storage.exists("invalid://uri")).rejects.toThrow(
        "Invalid URI",
      );
    });
  });
