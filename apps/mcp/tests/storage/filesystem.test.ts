...
describe("FileSystemResourceStorage", () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `pulse-test-${Date.now()}`);
  ...
  describe("write", () => {
    ...
    it("should write to the appropriate subdirectory based on resourceType", async () => {
      ...
      const cleanedUri = await storage.write(url, content, {
        resourceType: "cleaned",
      });
      const extractedUri = await storage.write(url, content, {
  ...
  describe("list", () => {
    ...
    it("should ignore non-markdown files", async () => {
      ...
      await fs.writeFile(
        path.join(testDir, "raw", "ignore.txt"),
        "Should be ignored",
      );
    ...
    it("should list resources from all subdirectories", async () => {
      await storage.write("https://example.com/raw", "Raw content", {
        resourceType: "raw",
      });
  ...
  describe("read", () => {
    ...
    it("should throw error for non-existent resource", async () => {
      await expect(storage.read("file:///non-existent")).rejects.toThrow(
        "Resource not found",
      );
    });

    it("should throw error for invalid URI", async () => {
      await expect(storage.read("invalid://uri")).rejects.toThrow(
        "Invalid file URI",
      );
    });
  });
  ...
  describe("delete", () => {
    ...
    it("should throw error when deleting non-existent resource", async () => {
      await expect(storage.delete("file:///non-existent")).rejects.toThrow(
        "Resource not found",
      );
    });
  });
  ...
  describe("findByUrl", () => {
    it("should return empty array for URL with no resources", async () => {
      const resources = await storage.findByUrl(
    ...
    it("should return resources sorted by timestamp descending", async () => {
      ...
      const timestamps = resources.map((r) =>
        new Date(r.metadata.timestamp).getTime(),
      );
