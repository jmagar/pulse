...
class TestScrapeCacheService:
    ...
    async def test_cache_key_differs_for_different_extract_query(
        ...
        key1 = cache_service.compute_cache_key(
            **params_base,
            extract="the author"
        )
        key2 = cache_service.compute_cache_key(
            **params_base,
            extract="the date"
        )
    ...
    async def test_cache_key_same_url_no_extract(
        self,
        cache_service: ScrapeCacheService
    ) -> None:
    ...
    async def test_invalidate_url_removes_all_entries(
        ...
        # Invalidate URL
        deleted_count = await cache_service.invalidate_url(
            session=db_session,
            url=url
        )
    ...
    async def test_url_hash_computation(
        self,
        cache_service: ScrapeCacheService
    ) -> None:
