...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        # Unique constraint
        sa.UniqueConstraint(
            "crawl_session_id", "url", "content_hash", name="uq_content_per_session_url"
        ),
        schema="webhook",
    )

    # Indexes (using idx_ prefix to match existing pattern)
    op.create_index("idx_scraped_content_url", "scraped_content", ["url"], schema="webhook")
    op.create_index(
        "idx_scraped_content_session", "scraped_content", ["crawl_session_id"], schema="webhook"
    )
    op.create_index(
        "idx_scraped_content_hash", "scraped_content", ["content_hash"], schema="webhook"
    )
    op.create_index(
        "idx_scraped_content_created", "scraped_content", ["created_at"], schema="webhook"
    )
