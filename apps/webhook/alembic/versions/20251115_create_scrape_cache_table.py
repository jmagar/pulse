...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('url_hash', sa.Text(), nullable=False, comment='SHA-256 hash of URL for fast lookups'),
        ...
        sa.Column('cleaned_content', sa.Text(), nullable=True, comment='Cleaned Markdown/text after processing'),
        sa.Column('extracted_content', sa.Text(), nullable=True, comment='LLM-extracted content'),
        sa.Column('extract_query', sa.Text(), nullable=True, comment='LLM extraction query used (for cache key)'),

        # Metadata
        sa.Column('source', sa.String(50), nullable=False, comment='Scraping source: firecrawl, native'),
        sa.Column('content_type', sa.String(100), nullable=True, comment='MIME type of content'),
        sa.Column('content_length_raw', sa.Integer(), nullable=True, comment='Length of raw_content in characters'),
        sa.Column('content_length_cleaned', sa.Integer(), nullable=True, comment='Length of cleaned_content in characters'),
        sa.Column('content_length_extracted', sa.Integer(), nullable=True, comment='Length of extracted_content in characters'),

        # Screenshot support
        sa.Column('screenshot', sa.LargeBinary(), nullable=True, comment='Base64 decoded screenshot binary'),
        sa.Column('screenshot_format', sa.String(20), nullable=True, comment='Screenshot MIME type: image/png'),

        # Scraping details
        sa.Column('strategy_used', sa.String(50), nullable=True, comment='Specific strategy that succeeded'),
        sa.Column('scrape_options', postgresql.JSONB(astext_type=sa.Text()), nullable=True,
                 comment='Full request options for debugging'),

        # Cache control
        sa.Column('scraped_at', sa.DateTime(timezone=True), nullable=False,
                 server_default=sa.text('now()'), comment='When content was scraped'),
