...
down_revision: str | Sequence[str] | None = "376d1cbc1ea8"
...
def upgrade() -> None:
    ...
    op.alter_column("crawl_sessions", "crawl_id", new_column_name="job_id", schema="webhook")

    # Rename crawl_url to base_url (more generic for all operations)
    op.alter_column("crawl_sessions", "crawl_url", new_column_name="base_url", schema="webhook")
    ...
    op.execute(
        "UPDATE webhook.crawl_sessions SET operation_type = 'crawl' WHERE operation_type IS NULL"
    )

    # Make operation_type non-nullable now that defaults are set
    op.alter_column("crawl_sessions", "operation_type", nullable=False, schema="webhook")
...
def downgrade() -> None:
    ...
    # Rename columns back
    op.alter_column("crawl_sessions", "base_url", new_column_name="crawl_url", schema="webhook")
    op.alter_column("crawl_sessions", "job_id", new_column_name="crawl_id", schema="webhook")
